...
#############################################################################
def predict(iFile,step,maxLineNumber,needPreprocess):
#iFile: the pair file; step: interval; maxLineNumber: used for separate groups
  baseFile="/var/lib/tomcat5/webapps/slap/temp/"
...
def submitJobs(iFile,oFile):
  ...
  jobs = []
  for input in inputs:
    jobs.append(job_server.submit(predict,(iFile,interval,input,needPreprocess), (predictSLAP,getNodeID,), ("pathfinder_rank5","odbc6",)))

  outputData=''
  #fout = open(baseFile+iFile+".slap","w")
  fout = open(oFile,"w")
...
if __name__=='__main__':
  ...
  if (len(sys.argv) > 2):
    ...
    print ("ERROR: Syntax: %s ifile ofile"%PROG)
