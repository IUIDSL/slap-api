...
def generalSimilarSearch(originalStartName, startnames, originalEndName,endnames, op=2, maxL=3, path2 = ""):
	...
	for startname in startnames:
		...
		for endname in endnames:
			...
			for key in output3[1].keys() :
				newkey=key
				if startname!=originalStartName:
					newkey = " "+originalStartName+" "+"http://chem2bio2rdf.org/pubchem/resource/neighbor"+" "+newkey
				if endname!=originalEndName:
					newkey =newkey[1:(len(newkey)-1)]+"http://chem2bio2rdf.org/pubchem/resource/neighbor"+" "+originalEndName+" ."
	...
	output4=makeNetwork(paths,0.05)
