...
def getPathPvalue(mypath,path2=""):
	...
	for key in mypath.keys():
		...
		if pattern in expectMeanTable:
			...
			expected_mean=-9.2
			expected_sd=2
		if float(mypath[key])> float(expected_mean) :
...
def generalSearch2(startname, endname, op=2, maxL=3, path2 = ""):
	...
	elif z_score<=195 and z_score>56 :
		result.append("weak")
	...
	if z_score>0:
		r = robjects.r
		#p_value=r.pnorm(log(z_score),mean=0.88,sd=1.03)[0]
	else:
		p_value=0
...
def generalSearchPPI(startname, endname, op=2, maxL=2, path2 = ""):
	file1 = "/var/www/html/rest/Chem2Bio2RDF/slap/dicts/node2id.txt"
